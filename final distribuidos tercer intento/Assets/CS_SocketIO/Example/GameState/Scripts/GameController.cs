...
public class GameController : MonoBehaviour
{
    ...
    [SerializeField]
    private Transform notCoinsContainer;
    [SerializeField]
    private GameObject PlayerPrefab;
    [SerializeField]
    private GameObject CoinPrefab;
    [SerializeField]
    private GameObject notCoinPrefab;

    private GameState State;
    private Dictionary<string, Transform> PlayersToRender;
    private Dictionary<string, Transform> CoinsToRender;
    private Dictionary<string, Transform> notCoinsToRender;
    internal void StartGame(GameState state)
    {
        ...
        notCoinsToRender = new Dictionary<string, Transform>();
    ...
    void Update()
    {
        if (State != null)
        {
            ...
            foreach (notCoin notcoin in State.notCoins)
            {
                if (notCoinsToRender.ContainsKey(notcoin.Id))
                {
                    notCoinsToRender[notcoin.Id].position = new Vector2(notcoin.x, notcoin.y);
                }
                else
                {
                    InstantiatenotCoin(notcoin);
                }
            }
            var notcoinsToDelete = notCoinsToRender.Where(item => !State.notCoins.Any(notcoin => notcoin.Id == item.Key)).ToList();

            foreach (var notcoinItem in notcoinsToDelete)
            {
                Destroy(notcoinItem.Value.gameObject);
                notCoinsToRender.Remove(notcoinItem.Key);
            }
    ...
    private void InstantiatenotCoin(notCoin notcoin)
    {
        GameObject notcoinGameObject = Instantiate(notCoinPrefab, notCoinsContainer);
        notcoinGameObject.transform.position = new Vector2(notcoin.x, notcoin.y);
        notcoinGameObject.GetComponent<GamenotCoin>().Id = notcoin.Id;

        notCoinsToRender[notcoin.Id] = notcoinGameObject.transform;
    }
